    ...
    public double aplicarDescuento(Cliente cli) {
        ...
        for (Localizador local : cli.getLocalizadores()) {
            if (local.getReservas().stream().map(r -> r.getTipoReserva()).collect(Collectors.toSet()).size() == 4) {
                // if(local.getReservas().stream().map(r ->
                // r.getTipoReserva()).collect(Collectors.toSet()).containsAll(List.of()))
                total += local.getTotal() * 0.9;
            } else if (local.getReservas().stream().map(r -> r.getTipoReserva()).filter(t -> t.equals("hotel"))
                    .count() >= 2 ||
                    local.getReservas().stream().map(r -> r.getTipoReserva()).filter(t -> t.equals("boletos"))
                            .count() >= 2) {
