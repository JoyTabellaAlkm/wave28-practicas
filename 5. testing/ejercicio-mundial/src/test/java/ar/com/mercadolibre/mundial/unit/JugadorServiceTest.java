...
class JugadorServiceTest {
    ...
    @Test
    public void validarCreacionJugador(){
        Jugador jugador = new Jugador(1, "Lionel Messi", "Argentina", 91);

        when(jugadorRepository.save(jugador)).thenReturn(true);

        boolean result = jugadorService.crearJugador(jugador);

        Assertions.assertTrue(result);
    }

    @Test
    public void validarEdicionJugador() {
        Jugador jugador = new Jugador(1, "Lionel Scaloni", "Argentina", 74);

        when(jugadorRepository.update(jugador)).thenReturn(true);

        boolean result = jugadorService.editarJugador(jugador);

        Assertions.assertTrue(result);
    }

    @Test
    public void validarEliminacionJugador() {
        int idJugador = 2;

        when(jugadorRepository.delete(idJugador)).thenReturn(true);

        boolean result = jugadorService.eliminarJugador(2);
