...
class JugadorServiceTest {
    ...
    @Test
    @DisplayName("Validar que un jugador se cree")
    void validarJugadorCreado() {
        List <Jugador> jugadores = jugadorRepository.cargarJugadores();
        List<Jugador> jugadoresModificado = jugadores.stream()
                .map(j -> j).collect(Collectors.toList());
        int cantidadJugadores = jugadores.size();
        Jugador jugadorNuevo = new Jugador(29, "Lionel Messi", "Argentina", 91);
        jugadoresModificado.add(jugadorNuevo);
        Mockito.when(jugadorRepository.crearUnJugador(jugadorNuevo)).thenReturn(jugadorNuevo);
        jugadorService.create(jugadorNuevo);
        assertNotNull(jugadorNuevo, "El jugador Messi no debería ser null");
        assertEquals(cantidadJugadores+1, jugadoresModificado.size(), "La cantidad de jugadores debe ser mayor al agregar un jugador");
    }
}
