...
class JugadorServiceTest {
    ...
    @Test
    @DisplayName("Validar que el jugador se crea")
    void validarCreacionJugador() {
        List<Jugador> jugadores = jugadorService.obtenerJugadoresOrdenadosPorGoles();
        List<Jugador> jugadoresLuegoCreacion = jugadores.stream().map(j -> j).collect(Collectors.toList());
        int cantidadJugadores = jugadores.size();

        Jugador jugadorACrear = new Jugador(21, "Luis Diaz", "Colombia", 50);
        jugadoresLuegoCreacion.add(jugadorACrear);

        Mockito.when(jugadorRepository.crearJugador(jugadorACrear)).thenReturn(jugadorACrear);

        Jugador jugadorCreado = jugadorService.crearJugador(jugadorACrear);


        assertEquals(jugadorACrear, jugadorCreado);
        assertEquals(cantidadJugadores+1, jugadoresLuegoCreacion.size(), "La cantidad de jugadores debe ser mayor al agregar un jugador");
    }
}
