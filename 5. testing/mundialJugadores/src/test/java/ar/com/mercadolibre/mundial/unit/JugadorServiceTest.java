...
class JugadorServiceTest {
    ...
    void setUp() {
        ...
        lenient().when(jugadorRepository.cargarJugadores()).thenReturn(jugadoresMock);
    }
    ...
    @Test
    @DisplayName("Validar que se agrega un jugador correctamente")
    void validarAgregarJugadorPepito() {
        Jugador pepito = new Jugador(21, "Pepito", "Uruguay", 5);

        when(jugadorRepository.agregar(pepito)).thenReturn(true);

        boolean agregado = jugadorService.agregarJugador(pepito);

        assertTrue(agregado);
    }

    @Test
    @DisplayName("Validar que no se agrega un jugador existente")
    void validarAgregarJugadorExistente() {
        Jugador pepito = new Jugador(1, "Lionel Messi", "Argentina", 91);

        when(jugadorRepository.agregar(pepito)).thenReturn(false);
    ...
    @Test
    @DisplayName("Validar que se elimina un jugador existente correctamente")
    void validarBorrarJugadorExistente() {
        Jugador messi = new Jugador(1, "Lionel Messi", "Argentina", 91);

        when(jugadorRepository.borrar(messi)).thenReturn(true);

        boolean borrado = jugadorService.eliminarJugador(messi.getId());

        assertTrue(borrado);
    }

    @Test
    @DisplayName("Validar que no se elimina un jugador no existente")
    void validarBorrarJugadorNoExistente() {
        Jugador pepito = new Jugador(100, "Pepito", "Argentina", 9);

        assertThrows(Error.class, () -> jugadorService.eliminarJugador(pepito.getId()));
    }
}
