...
class JugadorServiceTest {
    ...
    @Test
    @DisplayName("5- Validar que se agregue un jugador correctamente")
    @Order(5)
    void whenAddJugador_thenReturnMensaje(){
        //ARRANGE
        JugadorDTO jugadorDTO = new JugadorDTO(200,"Mutombo","Noruega",29);
        Jugador jugador = objectMapper.convertValue(jugadorDTO, Jugador.class);
        when(jugadorRepository.exist(jugadorDTO.getId())).thenReturn(false);
        // Simulamos que el método save devuelve el jugador guardado
        when(jugadorRepository.save(any(Jugador.class))).thenReturn(jugador);

        //ACT
        MensajeDTO mensajeActual = jugadorService.registrarJugador(jugadorDTO);

        //ASSERT
        assertEquals("Jugador registrado",mensajeActual.getMensaje());
        verify(jugadorRepository, times(1)).save(any(Jugador.class));
    }

    @Test
    @DisplayName("6- Validar que que se capture IllegalArgumentException al ingresar un id existente")
    @Order(6)
    void whenAddJugador_JugadorExists_thenReturnIllegalArgumentException(){
        //ARRANGE
        JugadorDTO jugadorDTO = new JugadorDTO(200,"Mutombo","Noruega",29);
        when(jugadorRepository.exist(200)).thenReturn(true);

        //ACT //ASSERT
        assertThrows(IllegalArgumentException.class,
                () -> jugadorService.registrarJugador(jugadorDTO),
                "Se esperaba una excepción IllegalArgumentException");
        verify(jugadorRepository, times(1)).exist(jugadorDTO.getId());
